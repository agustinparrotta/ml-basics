...
class Trader:
    ...
    def act(self, state):
        ...
        """
        offset = 0 if len(self.book) > 0 else -1  # Para evitar Sell si no hay posiciones

        if random.random() <= self.epsilon:
            return (random.randrange(self.action_size + offset), False)

        views_strength = self.model.predict(state)
        return (np.argmax(views_strength[0][:self.action_size + offset]), True)
