...
class BinanceWrapper(BinanceBase):
    ...
    def get_klines(self, symbol:str, interval:str, limit:int=100, start_time:datetime=None, end_time:datetime=None)->pd.DataFrame:
        ...
        if r.status_code == 200:
            ...
            df = pd.DataFrame(r.json(), columns=self.KLINES_COLUMNS).drop('ignore', axis=1).astype(self.KLINES_TYPES)

            df.loc[:, ['open_time', 'close_time']] = (df.loc[:, ['open_time', 'close_time']]/1000).applymap(datetime.fromtimestamp)
    ...
    def get_price_change_statistics(self, symbol:str=None)->pd.DataFrame:
        ...
        if r.status_code == 200:
            ...
            df = pd.DataFrame(j, index=[0] if symbol else None).astype(self.PRICE_CHANGE_STATISTICS_TYPES)

            df.loc[:, ['openTime', 'closeTime']] = (df.loc[:, ['openTime', 'closeTime']]/1000).applymap(datetime.fromtimestamp)
    ...
    def get_last_price(self, symbol:str=None)->pd.DataFrame:
        ...
        if r.status_code == 200:
            ...
            df = pd.DataFrame(j, index=[0] if symbol else None).astype(self.LAST_PRICE_TYPES)
    ...
    def get_book_ticker(self, symbol:str=None):
        ...
        if r.status_code == 200:
            ...
            df = pd.DataFrame(j, index=[0] if symbol else None).astype(self.BOOK_TICKER_TYPES)
