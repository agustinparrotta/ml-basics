...
lr_score = np.max(np.mean(lr.scores_[1], axis=1))
...
# Metricas: https://scikit-learn.org/stable/modules/generated/sklearn.neighbors.DistanceMetric.html
...
# %% SVM usando NN equivalente
def build_nn():
    model = tfk.Sequential([tfk.layers.Dense(1, activation='linear')])
    model.compile(optimizer='SGD', loss='hinge')
    return model


svc_nn = KerasClassifier(build_fn=build_nn, epochs=5, batch_size=20,
                         verbose=False)
